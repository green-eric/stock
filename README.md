...
| 模块 | 说明 |
|------|------|
| **数据采集Agent** | 多数据源（Tushare、BaoStock）真实热点板块、资金流向数据，智能数据源切换机制，5分钟间隔（仅交易时段） |
| **技术分析Agent** | 基于配置文件监控列表的技术分析，多数据源支持（Tushare、BaoStock），15分钟间隔（仅交易时段） |
| **系统监控Agent** | 实时监控Agent状态，30分钟间隔（全天运行） |
| **钉钉集成** | 签名校验、消息推送、速率限制、数据源信息展示 |
| **文件存储** | JSON格式数据文件，按时间戳保存 |
| **进程管理** | Shell脚本启动/停止/状态监控 |

#### ⚠️ 注意事项
1. **多数据源**: 数据采集Agent使用多数据源（Tushare、BaoStock），智能切换机制确保数据获取可靠性
2. **交易时段限制**: 数据采集和技术分析仅在09:00-15:00运行
3. **无实盘交易**: 仅信号推送，不执行实际买卖操作
4. **真实数据源**: 技术分析基于真实价格数据，多数据源支持，智能切换机制确保数据可靠性
5. **可扩展监控**: 技术分析监控股票列表可通过config/watch_list.txt配置文件扩展

#### 🚀 后续演进
- **真实数据源**: 已接入Tushare、BaoStock获取实时行情（数据采集Agent）
...
- **📱 钉钉集成**: 实时消息推送，支持签名校验和关键词过滤，数据源信息展示
- **⚡ 超短线优化**: 专为1-3天持股周期设计的热点追踪策略
- **🔐 风险控制**: 完整的仓位管理、止损机制和熔断保护
- **🔄 自动化运行**: 定时任务，7×24小时不间断监控
- **🌐 多数据源支持**: 集成Tushare、BaoStock多个数据源，智能切换机制确保数据可靠性（数据采集和技术分析Agent均支持）
...
- **功能**:
  ...
  - 智能数据源切换机制
  - 基于价格大于等于5日均线价的买入信号生成（最多发送3个买入信号）
  - 风险预警
...
```json
{
  "agent_settings": {
    ...
    "signal_fusion": {
      "enabled": true,
      "buy_condition": "price_above_ma5",  // 买入条件：价格大于等于5日均线
      "sell_condition": "stop_loss"       // 卖出条件：止损
    }
  }
}
```
...
# 监控股票列表
# 修改 ./config/watch_list.txt 文件，格式：股票代码,股票名称
# 示例：603113,金能科技
```
