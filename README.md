...
| 模块 | 状态 | 说明 |
|------|------|------|
| **数据采集Agent** | ✅ 运行中 | 多数据源（AKShare、新浪财经、东方财富）真实热点板块、资金流向数据，智能数据源切换机制，5分钟间隔（仅交易时段） |
| **技术分析Agent** | ✅ 运行中 | 基于配置文件监控列表的技术分析，多数据源支持（AKShare、新浪财经、东方财富），15分钟间隔（仅交易时段） |
...
#### ⚠️ 注意事项
1. **多数据源**: 数据采集Agent使用多数据源（AKShare、新浪财经、东方财富），智能切换机制确保数据获取可靠性
...
- **🌐 多数据源支持**: 集成AKShare、新浪财经、东方财富多个数据源，智能切换机制确保数据可靠性（数据采集和技术分析Agent均支持）
